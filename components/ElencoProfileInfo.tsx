    ...
    <div className="flex flex-1 flex-col px-8 pb-24 pt-28 md:px-14 md:pb-32 md:pt-32">
      ...
      <div className="flex flex-col gap-2 text-sm text-neutral-600">
        {artist.anoNascimento != null && (
          <p>
            <span className="font-medium text-neutral-900">{t.labels.nascimento}:</span>{' '}
            {artist.anoNascimento}
          </p>
        )}
        {artist.altura && (
          <p>
            <span className="font-medium text-neutral-900">{t.labels.altura}:</span>{' '}
            {artist.altura}
          </p>
        )}
        {artist.cidadeEstado && (
          <p>
            <span className="font-medium text-neutral-900">{t.labels.local}:</span>{' '}
            {artist.cidadeEstado}
          </p>
        )}
        {artist.idiomas && artist.idiomas.length > 0 && (
          <p>
            <span className="font-medium text-neutral-900">{t.labels.idiomas}:</span>{' '}
            {artist.idiomas.join(', ')}
          </p>
        )}
      </div>
      ...
      {artist.biografia && (
        <>
          ...
          {typeof artist.biografia === 'string' ? (
            <p className="max-w-[52ch] leading-relaxed text-neutral-600" style={{ fontSize: '0.95rem' }}>
              {artist.biografia}
            </p>
          ) : (
            <div className="max-w-[52ch] leading-relaxed text-neutral-600" style={{ fontSize: '0.95rem' }}>
              <PortableText
